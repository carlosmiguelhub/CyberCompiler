...
function RightSidebar({ profile }) {
  ...
  return (
   <aside
  className={`relative border-l border-slate-200 bg-white/80 text-slate-900 backdrop-blur-md transition-all transition-colors duration-300 dark:border-slate-800 dark:bg-slate-950/90 dark:text-slate-50 ${
    isOpen ? "w-72" : "w-12"
  }`}
>
      ...
      <button
        ...
        className="absolute -left-4 top-4 z-10 flex h-8 w-8 items-center justify-center rounded-full border border-slate-200 bg-white text-xs text-slate-700 shadow-md hover:border-slate-300 hover:bg-slate-50 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-200 dark:hover:border-slate-600 dark:hover:bg-slate-900"
      >
        {isOpen ? "›" : "‹"}
      </button>

      <div className={isOpen ? "h-full px-4 py-4" : "h-full px-1 py-4"}>
